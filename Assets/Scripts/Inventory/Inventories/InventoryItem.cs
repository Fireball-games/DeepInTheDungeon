...
namespace Scripts.Inventory.Inventories
{
    ...
    public abstract class InventoryItem : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        public static InventoryItem GetFromID(string itemID)
        {
            if (_itemLookupCache == null)
            {
                ...
                foreach (InventoryItem item in itemList)
                {
                    ...
                    {
                        Debug.LogError(string.Format("Looks like there's a duplicate GameDevTV.UI.InventorySystem ID for objects: {0} and {1}", _itemLookupCache[item.itemID], item));
                        continue;
                    }
        ...
        void ISerializationCallbackReceiver.OnBeforeSerialize()
        {
            ...
            {
                itemID = global::System.Guid.NewGuid().ToString();
            }
        }
